...
[ApiController]
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/bus/seat-holds")]
[Authorize]
public sealed class BusTripSeatHoldsController : ControllerBase
{
    ...
    public async Task<IActionResult> HoldSeats([FromBody] HoldSeatsRequest req, CancellationToken ct = default)
    {
        ...
        var validSeatCount = await _db.Seats.IgnoreQueryFilters()
            .CountAsync(x =>
                x.TenantId == tenantId &&
    ...
    {
        holdToken = (holdToken ?? string.Empty).Trim();
    ...
    public async Task<IActionResult> ListByTrip(Guid tripId, [FromQuery] bool includeExpired = false, CancellationToken ct = default)
    {
        ...
        var query = _db.BusTripSeatHolds.IgnoreQueryFilters()
            .Where(x => x.TenantId == trip.TenantId && x.TripId == tripId && !x.IsDeleted);
