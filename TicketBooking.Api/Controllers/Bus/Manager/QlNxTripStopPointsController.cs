// FILE #282: TicketBooking.Api/Controllers/QlNxTripStopPointsController.cs
...
[ApiController]
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/qlnx/bus")]
[Authorize(Roles = $"{RoleNames.Admin},{RoleNames.QLNX}")]
public sealed class QlNxTripStopPointsController : ControllerBase
{
    private readonly AppDbContext _db;
    private readonly ITenantContext _tenantContext;

    public QlNxTripStopPointsController(AppDbContext db, ITenantContext tenantContext)
    {
        _db = db;
    ...
    public async Task<IActionResult> ListPickupPoints(
        Guid tripStopTimeId,
        ...
        CancellationToken ct = default)
    {
        ...
        var tripStopTimeExists = await _db.BusTripStopTimes.IgnoreQueryFilters()
            .AnyAsync(x => x.Id == tripStopTimeId && x.TenantId == _tenantContext.TenantId, ct);

        if (!tripStopTimeExists)
            return NotFound(new { message = "TripStopTime not found in this tenant." });

        IQueryable<TripStopPickupPoint> query = _db.BusTripStopPickupPoints
            .Where(x => x.TripStopTimeId == tripStopTimeId);
    ...
    public async Task<IActionResult> CreatePickupPoint(
        Guid tripStopTimeId,
        [FromBody] UpsertPickupPointRequest req,
        CancellationToken ct = default)
    {
        EnsureTenantScope();
        ValidatePickup(req);

        var tripStopTime = await _db.BusTripStopTimes.IgnoreQueryFilters()
            .FirstOrDefaultAsync(x => x.Id == tripStopTimeId && x.TenantId == _tenantContext.TenantId && !x.IsDeleted, ct);

        if (tripStopTime is null)
            return BadRequest(new { message = "TripStopTimeId is invalid for this tenant." });

        if (req.IsDefault)
        {
            var others = await _db.BusTripStopPickupPoints.IgnoreQueryFilters()
                .Where(x =>
                    x.TripStopTimeId == tripStopTimeId &&
        ...
        CancellationToken ct = default)
    {
        ...
        if (req.IsDefault && !entity.IsDefault)
        {
            var others = await _db.BusTripStopPickupPoints.IgnoreQueryFilters()
                .Where(x =>
                    x.TripStopTimeId == entity.TripStopTimeId &&
                    x.TenantId == _tenantContext.TenantId &&
                    !x.IsDeleted &&
                    x.IsDefault)
                .ToListAsync(ct);
    ...
    public async Task<IActionResult> DeletePickupPoint(
        Guid id,
        CancellationToken ct = default)
    {
        EnsureTenantScope();

        var entity = await _db.BusTripStopPickupPoints
            .FirstOrDefaultAsync(x => x.Id == id, ct);

        if (entity is null)
            return NotFound(new { message = "Pickup point not found." });
    ...
    [HttpPost("trip-stop-pickup-points/{id:guid}/restore")]
    public async Task<IActionResult> RestorePickupPoint(
        Guid id,
        CancellationToken ct = default)
    {
        EnsureTenantScope();
    ...
    public async Task<IActionResult> ListDropoffPoints(
        Guid tripStopTimeId,
        [FromQuery] bool includeDeleted = false,
        CancellationToken ct = default)
    {
        ...
        IQueryable<TripStopDropoffPoint> query = _db.BusTripStopDropoffPoints
            .Where(x => x.TripStopTimeId == tripStopTimeId);
    ...
    public async Task<IActionResult> CreateDropoffPoint(
        Guid tripStopTimeId,
        [FromBody] UpsertDropoffPointRequest req,
        CancellationToken ct = default)
    {
        EnsureTenantScope();
        ValidateDropoff(req);
    ...
    public async Task<IActionResult> DeleteDropoffPoint(
        Guid id,
        CancellationToken ct = default)
    {
        EnsureTenantScope();

        var entity = await _db.BusTripStopDropoffPoints
            .FirstOrDefaultAsync(x => x.Id == id, ct);

        if (entity is null)
            return NotFound(new { message = "Dropoff point not found." });
    ...
    [HttpPost("trip-stop-dropoff-points/{id:guid}/restore")]
    public async Task<IActionResult> RestoreDropoffPoint(
        Guid id,
        CancellationToken ct = default)
    {
        EnsureTenantScope();
