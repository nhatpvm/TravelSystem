// FILE #069 (UPDATE): TicketBooking.Api/Controllers/BusRoutesAdminController.cs
...
[ApiController]
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/admin/bus/routes")]
[Authorize(Roles = RoleNames.Admin)]
public sealed class BusRoutesAdminController : ControllerBase
{
    private readonly AppDbContext _db;
    private readonly ITenantContext _tenantContext;

    public BusRoutesAdminController(AppDbContext db, ITenantContext tenantContext)
    {
        _db = db;
    ...
    public async Task<IActionResult> Create([FromBody] UpsertRouteRequest req, CancellationToken ct = default)
    {
        ...
        if (exists) return Conflict(new { message = "Route Code already exists in this tenant." });
    ...
    public async Task<IActionResult> Update(Guid id, [FromBody] UpsertRouteRequest req, CancellationToken ct = default)
    {
        ...
        if (entity is null) return NotFound(new { message = "Route not found in this tenant." });
    ...
    public async Task<IActionResult> ReplaceStops(Guid id, [FromBody] UpsertRouteStopsRequest req, CancellationToken ct = default)
    {
        ...
        if (route is null) return NotFound(new { message = "Route not found in this tenant." });
    ...
    public async Task<IActionResult> SoftDelete(Guid id, CancellationToken ct = default)
    {
        ...
        var entity = await _db.BusRoutes.FirstOrDefaultAsync(
    ...
    public async Task<IActionResult> Restore(Guid id, CancellationToken ct = default)
    {
        ...
        if (entity is null) return NotFound(new { message = "Route not found." });
