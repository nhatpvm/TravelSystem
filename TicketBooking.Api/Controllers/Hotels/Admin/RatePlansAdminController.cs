...
// FILE #209: TicketBooking.Api/Controllers/Hotels/RatePlansAdminController.cs
...
[ApiController]
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/admin/rate-plans")]
[Authorize(Roles = RoleNames.Admin)]
public sealed class RatePlansAdminController : ControllerBase
{
    private readonly AppDbContext _db;
    private readonly ITenantContext _tenant;

    public RatePlansAdminController(AppDbContext db, ITenantContext tenant)
    {
        _db = db;
        _tenant = tenant;
    }
    ...
    [HttpGet]
    public async Task<ActionResult<AdminRatePlanPagedResponse<AdminRatePlanListItemDto>>> List(
        ...
        CancellationToken ct = default)
    {
        ...
        if (!resolvedTenantId.HasValue && !string.IsNullOrWhiteSpace(tenantCode))
        {
            ...
            {
                return Ok(new AdminRatePlanPagedResponse<AdminRatePlanListItemDto>
                {
                    Page = page,
                    PageSize = pageSize,
                    Total = 0,
        ...
        IQueryable<RatePlan> query = includeDeleted
            ? _db.RatePlans.IgnoreQueryFilters()
            : _db.RatePlans.Where(x => !x.IsDeleted);
        ...
        var items = await query.AsNoTracking()
            ...
            .Take(pageSize)
            .Select(x => new AdminRatePlanListItemDto
            {
                Id = x.Id,
                TenantId = x.TenantId,
                HotelId = x.HotelId,
    ...
    [HttpGet("{id:guid}")]
    public async Task<ActionResult<AdminRatePlanDetailDto>> GetById(
        Guid id,
        ...
        CancellationToken ct = default)
    {
        ...
        var roomTypeMappings = await _db.RatePlanRoomTypes.IgnoreQueryFilters()
            .Where(x => x.TenantId == entity.TenantId && x.RatePlanId == id && !x.IsDeleted)
            .Join(
                _db.RoomTypes.IgnoreQueryFilters().Where(x => x.TenantId == entity.TenantId && !x.IsDeleted),
                m => m.RoomTypeId,
                r => r.Id,
                (m, r) => new AdminRatePlanRoomTypeDto
                {
                    Id = m.Id,
        ...
        var policy = await _db.RatePlanPolicies.IgnoreQueryFilters()
            .Where(x => x.TenantId == entity.TenantId && x.RatePlanId == id && !x.IsDeleted)
            .Select(x => new AdminRatePlanPolicyDto
            {
                Id = x.Id,
        ...
        return Ok(new AdminRatePlanDetailDto
        {
            Id = entity.Id,
    ...
    [HttpPost]
    public async Task<ActionResult<AdminCreateRatePlanResponse>> Create(
        [FromBody] AdminCreateRatePlanRequest req,
        CancellationToken ct = default)
    {
        ...
        if (req.CancellationPolicyId.HasValue)
        {
            ...
            if (!ok) return BadRequest(new { message = "CancellationPolicyId is invalid in current switched tenant." });
        }
        ...
        if (req.CheckInOutRuleId.HasValue)
        {
            ...
            if (!ok) return BadRequest(new { message = "CheckInOutRuleId is invalid in current switched tenant." });
        }
        ...
        if (req.PropertyPolicyId.HasValue)
        {
            ...
            if (!ok) return BadRequest(new { message = "PropertyPolicyId is invalid in current switched tenant." });
        }
        ...
        return CreatedAtAction(
            ...
            new AdminCreateRatePlanResponse { Id = entity.Id });
    }

    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(
        Guid id,
        [FromBody] AdminUpdateRatePlanRequest req,
        CancellationToken ct = default)
    {
        RequireAdminWriteTenant();
        ValidateUpdate(req);
        ...
        if (entity is null)
            return NotFound(new { message = "Rate plan not found in current switched tenant." });
    ...
    private async Task ReplaceRoomTypeMappingsAsync(
        Guid tenantId,
        Guid ratePlanId,
        List<AdminRatePlanRoomTypeUpsertDto> items,
        DateTimeOffset now,
        Guid? userId,
    ...
    private async Task UpsertPolicyAsync(
        Guid tenantId,
        Guid ratePlanId,
        AdminRatePlanPolicyUpsertDto dto,
        DateTimeOffset now,
        Guid? userId,
        CancellationToken ct)
    {
    ...
    private static void ValidateCreate(AdminCreateRatePlanRequest req)
    {
    ...
    private static void ValidateUpdate(AdminUpdateRatePlanRequest req)
    {
...
public sealed class AdminRatePlanPagedResponse<T>
{
...
public sealed class AdminRatePlanListItemDto
{
...
public sealed class AdminRatePlanDetailDto
{
    public Guid Id { get; set; }
    public Guid TenantId { get; set; }
    ...
    public List<AdminRatePlanRoomTypeDto> RoomTypes { get; set; } = new();
    public AdminRatePlanPolicyDto? Policy { get; set; }
}

public sealed class AdminRatePlanRoomTypeDto
{
    public Guid Id { get; set; }
...
public sealed class AdminRatePlanPolicyDto
{
    public Guid Id { get; set; }
...
public sealed class AdminCreateRatePlanRequest
{
    public Guid HotelId { get; set; }
    public string? Code { get; set; }
    public string? Name { get; set; }
    ...
    public List<AdminRatePlanRoomTypeUpsertDto>? RoomTypes { get; set; }
    public AdminRatePlanPolicyUpsertDto? Policy { get; set; }
}

public sealed class AdminUpdateRatePlanRequest
{
    public Guid? HotelId { get; set; }
    public string? Code { get; set; }
    public string? Name { get; set; }
...
public sealed class AdminRatePlanRoomTypeUpsertDto
{
    public Guid RoomTypeId { get; set; }
    public decimal? BasePrice { get; set; }
...
public sealed class AdminRatePlanPolicyUpsertDto
{
    public string? PolicyJson { get; set; }
    public bool? IsActive { get; set; }
}

public sealed class AdminCreateRatePlanResponse
{
    public Guid Id { get; set; }
}
