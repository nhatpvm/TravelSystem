// FILE #207: TicketBooking.Api/Controllers/Hotels/AdminHotelsController.cs
...
[ApiController]
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/admin/hotels")]
...
public sealed class AdminHotelsController : ControllerBase
{
    ...
    public AdminHotelsController(AppDbContext db, ITenantContext tenant)
    {
        _db = db;
        _tenant = tenant;
    }
    ...
    [HttpGet]
    public async Task<ActionResult<AdminHotelPagedResponse<AdminHotelListItemDto>>> List(
        ...
        CancellationToken ct = default)
    {
        ...
        if (!resolvedTenantId.HasValue && !string.IsNullOrWhiteSpace(tenantCode))
        {
            ...
            if (!resolvedTenantId.HasValue)
                return Ok(new AdminHotelPagedResponse<AdminHotelListItemDto>
                {
                    Page = page,
                    PageSize = pageSize,
                    Total = 0,
        ...
        IQueryable<Hotel> query = includeDeleted
            ? _db.Hotels.IgnoreQueryFilters()
            : _db.Hotels.Where(x => !x.IsDeleted);
        ...
        var items = await query.AsNoTracking()
            ...
            .Take(pageSize)
            .Select(x => new AdminHotelListItemDto
            {
                Id = x.Id,
                TenantId = x.TenantId,
                Code = x.Code,
    ...
    [HttpGet("{id:guid}")]
    public async Task<ActionResult<AdminHotelDetailDto>> GetById(
        Guid id,
        ...
        CancellationToken ct = default)
    {
        ...
        return Ok(MapDetail(entity, tenantCode));
    }

    [HttpPost]
    public async Task<ActionResult<AdminCreateHotelResponse>> Create(
        [FromBody] AdminCreateHotelRequest req,
        CancellationToken ct = default)
    {
        ...
        if (codeExists)
            return Conflict(new { message = "Hotel code already exists in current tenant." });

        var slugExists = await _db.Hotels.IgnoreQueryFilters()
            .AnyAsync(x => x.TenantId == tenantId && x.Slug == slug, ct);

        if (slugExists)
            return Conflict(new { message = "Hotel slug already exists in current tenant." });
        ...
        return CreatedAtAction(
            ...
            new AdminCreateHotelResponse { Id = entity.Id });
    }

    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(
        Guid id,
        [FromBody] AdminUpdateHotelRequest req,
        CancellationToken ct = default)
    {
        RequireAdminWriteTenant();
    ...
    private static void ValidateCreate(AdminCreateHotelRequest req)
    {
        if (string.IsNullOrWhiteSpace(req.Code))
    ...
    private static void ValidateUpdate(AdminUpdateHotelRequest req)
    {
    ...
    private static AdminHotelDetailDto MapDetail(Hotel x, string? tenantCode) => new()
    {
        Id = x.Id,
        TenantId = x.TenantId,
...
public sealed class AdminHotelPagedResponse<T>
{
...
public sealed class AdminHotelListItemDto
{
...
public sealed class AdminHotelDetailDto
{
    public Guid Id { get; set; }
    public Guid TenantId { get; set; }
...
public sealed class AdminCreateHotelRequest
{
    public string? Code { get; set; }
    public string? Name { get; set; }
    public string? Slug { get; set; }
...
public sealed class AdminUpdateHotelRequest
{
    public string? Code { get; set; }
    public string? Name { get; set; }
    public string? Slug { get; set; }
...
public sealed class AdminCreateHotelResponse
{
    public Guid Id { get; set; }
}
