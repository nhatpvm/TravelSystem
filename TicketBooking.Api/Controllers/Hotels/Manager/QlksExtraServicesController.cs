// FILE #206: TicketBooking.Api/Controllers/Hotels/QlksExtraServicesController.cs
...
[ApiController]
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/qlks/extra-services")]
[Authorize(Roles = RoleNames.QLKS + "," + RoleNames.Admin)]
public sealed class QlksExtraServicesController : ControllerBase
{
    private readonly AppDbContext _db;
    private readonly ITenantContext _tenant;

    public QlksExtraServicesController(AppDbContext db, ITenantContext tenant)
    {
        _db = db;
        _tenant = tenant;
    }
    ...
    [HttpGet]
    public async Task<ActionResult<ExtraServicePagedResponse<ExtraServiceListItemDto>>> List(
        ...
        CancellationToken ct = default)
    {
        ...
        var items = await query.AsNoTracking()
            ...
            .Take(pageSize)
            .Select(x => new ExtraServiceListItemDto
            {
                Id = x.Id,
                TenantId = x.TenantId,
                HotelId = x.HotelId,
        ...
        return Ok(new ExtraServicePagedResponse<ExtraServiceListItemDto>
        {
            Page = page,
            PageSize = pageSize,
            Total = total,
    ...
    [HttpGet("{id:guid}")]
    public async Task<ActionResult<ExtraServiceDetailDto>> GetById(
        Guid id,
        [FromQuery] bool includeDeleted = false,
        CancellationToken ct = default)
    {
        ...
        if (entity is null)
            return NotFound(new { message = "Extra service not found." });
        ...
        var prices = await _db.ExtraServicePrices.IgnoreQueryFilters()
            ...
            .ThenBy(x => x.EndDate)
            .Select(x => new ExtraServicePriceDto
            {
                Id = x.Id,
        ...
        return Ok(new ExtraServiceDetailDto
        {
            Id = entity.Id,
    ...
    [HttpPost]
    public async Task<ActionResult<CreateExtraServiceResponse>> Create(
        [FromBody] CreateExtraServiceRequest req,
        CancellationToken ct = default)
    {
        if (!_tenant.HasTenant || !_tenant.TenantId.HasValue)
            return BadRequest(new { message = "Tenant context is required." });
        ...
        return CreatedAtAction(
            ...
            new CreateExtraServiceResponse { Id = entity.Id });
    }

    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(
        Guid id,
        [FromBody] UpdateExtraServiceRequest req,
        CancellationToken ct = default)
    {
        if (!_tenant.HasTenant || !_tenant.TenantId.HasValue)
            return BadRequest(new { message = "Tenant context is required." });
    ...
    public async Task<ActionResult<List<ExtraServicePriceDto>>> GetPrices(
        Guid id,
    ...
    public async Task<IActionResult> ReplacePrices(
        Guid id,
        [FromBody] ReplaceExtraServicePricesRequest req,
        CancellationToken ct = default)
    {
        if (!_tenant.HasTenant || !_tenant.TenantId.HasValue)
            return BadRequest(new { message = "Tenant context is required." });
    ...
    private async Task ReplacePricesAsync(
        Guid tenantId,
        Guid extraServiceId,
        List<ExtraServicePriceUpsertDto> items,
        DateTimeOffset now,
        Guid? userId,
    ...
    private static void ValidateCreate(CreateExtraServiceRequest req)
    {
    ...
    private static void ValidateUpdate(UpdateExtraServiceRequest req)
    {
    ...
    private static void ValidateReplacePrices(ReplaceExtraServicePricesRequest req)
    {
...
public sealed class ExtraServicePagedResponse<T>
{
...
public sealed class ExtraServiceListItemDto
{
...
public sealed class ExtraServiceDetailDto
{
    public Guid Id { get; set; }
    public Guid TenantId { get; set; }
    public Guid HotelId { get; set; }
    ...
    public List<ExtraServicePriceDto> Prices { get; set; } = new();
}

public sealed class ExtraServicePriceDto
{
    public Guid Id { get; set; }
...
public sealed class CreateExtraServiceRequest
{
    public Guid HotelId { get; set; }
    public string? Code { get; set; }
    public string? Name { get; set; }
    ...
    public string? MetadataJson { get; set; }
    public bool? IsActive { get; set; }
    public List<ExtraServicePriceUpsertDto>? Prices { get; set; }
}

public sealed class UpdateExtraServiceRequest
{
    public Guid? HotelId { get; set; }
    public string? Code { get; set; }
    public string? Name { get; set; }
...
public sealed class ExtraServicePriceUpsertDto
{
    public DateOnly StartDate { get; set; }
    public DateOnly EndDate { get; set; }
...
public sealed class ReplaceExtraServicePricesRequest
{
    public List<ExtraServicePriceUpsertDto> Prices { get; set; } = new();
}

public sealed class CreateExtraServiceResponse
{
    public Guid Id { get; set; }
}
