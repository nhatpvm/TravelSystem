//FILE: TicketBooking.Api/Controllers/Admin/FlightAirportsAdminController.cs
...
[ApiController]
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/admin/flight/airports")]
[Authorize(Roles = "Admin,QLVMM")]
public sealed class FlightAirportsAdminController : ControllerBase
{
    private readonly AppDbContext _db;
    private readonly ITenantContext _tenant;

    public FlightAirportsAdminController(AppDbContext db, ITenantContext tenant)
    {
        _db = db;
        _tenant = tenant;
    }

    public sealed class AirportUpsertRequest
    {
        ...
        CancellationToken ct = default)
    {
        ...
        IQueryable<Airport> query = _db.FlightAirports.AsNoTracking();
        ...
        if (!string.IsNullOrWhiteSpace(q))
        {
            ...
            query = query.Where(x =>
                ...
                (x.TimeZone != null && x.TimeZone.ToUpper().Contains(uq)));
        }
    ...
    public async Task<IActionResult> Create(
        [FromBody] AirportUpsertRequest req,
        CancellationToken ct = default)
    {
        if (!_tenant.HasTenant || _tenant.TenantId is null || _tenant.TenantId == Guid.Empty)
            return BadRequest(new { message = "Missing tenant context. Please provide X-TenantId." });
        ...
        var timeZone = TrimOrNull(req.TimeZone, 64);

        var locationExists = await _db.Locations
            .IgnoreQueryFilters()
            .AnyAsync(x => x.TenantId == tenantId && x.Id == req.LocationId && !x.IsDeleted, ct);

        if (!locationExists)
            return BadRequest(new { message = "LocationId not found in catalog locations for this tenant." });

        var codeExists = await _db.FlightAirports.IgnoreQueryFilters()
            .AnyAsync(x => x.TenantId == tenantId && x.Code == code, ct);

        if (codeExists)
            return Conflict(new { message = $"Airport code '{code}' already exists." });

        if (!string.IsNullOrWhiteSpace(iataCode))
        {
            var iataExists = await _db.FlightAirports.IgnoreQueryFilters()
        ...
        {
            var icaoExists = await _db.FlightAirports.IgnoreQueryFilters()
        ...
        CancellationToken ct = default)
    {
        ...
        var entity = await _db.FlightAirports.IgnoreQueryFilters()
        ...
        CancellationToken ct = default)
    {
        ...
        if (entity.IsDeleted)
        {
            ...
            if (codeExists)
                return Conflict(new { message = $"Cannot restore: airport code '{entity.Code}' already exists." });
