// FILE #246: TicketBooking.Api/Controllers/Tours/QlTourToursController.cs
...
[ApiController]
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/ql-tour/tours")]
[Authorize(Roles = $"{RoleNames.QLTour},{RoleNames.Admin}")]
public sealed class QlTourToursController : ControllerBase
{
    private readonly AppDbContext _db;
    private readonly ITenantContext _tenant;

    public QlTourToursController(AppDbContext db, ITenantContext tenant)
    {
        _db = db;
        _tenant = tenant;
    }
    ...
    [HttpGet]
    public async Task<ActionResult<QlTourTourPagedResponse>> List(
        ...
        CancellationToken ct = default)
    {
        ...
        IQueryable<Tour> query = includeDeleted
            ? _db.Tours.IgnoreQueryFilters().Where(x => x.TenantId == tenantId)
            : _db.Tours.Where(x => x.TenantId == tenantId && !x.IsDeleted);
        ...
        var items = await query
            ...
            .Take(pageSize)
            .Select(x => new QlTourTourListItemDto
            {
                Id = x.Id,
        ...
        return Ok(new QlTourTourPagedResponse
        {
            Page = page,
            PageSize = pageSize,
            Total = total,
    ...
    [HttpGet("{id:guid}")]
    public async Task<ActionResult<QlTourTourDetailDto>> GetById(
        Guid id,
        ...
        CancellationToken ct = default)
    {
        ...
        if (entity is null)
            return NotFound(new { message = "Tour not found in current tenant." });

        return Ok(MapDetail(entity));
    }

    [HttpPost]
    public async Task<ActionResult<QlTourCreateTourResponse>> Create(
        [FromBody] QlTourCreateTourRequest req,
        CancellationToken ct = default)
    {
        var tenantId = RequireTenant();
        ...
        return CreatedAtAction(
            ...
            new QlTourCreateTourResponse { Id = entity.Id });
    }

    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(
        Guid id,
        [FromBody] QlTourUpdateTourRequest req,
        CancellationToken ct = default)
    {
        var tenantId = RequireTenant();
    ...
    private async Task ValidateCreateAsync(Guid tenantId, QlTourCreateTourRequest req, CancellationToken ct)
    {
        ...
        if (duplicatedCode)
            throw new ArgumentException("Code already exists in current tenant.");

        var duplicatedSlug = await _db.Tours.IgnoreQueryFilters()
            .AnyAsync(x => x.TenantId == tenantId && x.Slug == slug, ct);

        if (duplicatedSlug)
            throw new ArgumentException("Slug already exists in current tenant.");
    }

    private async Task ValidateUpdateAsync(
        Guid tenantId,
        Guid currentId,
        QlTourUpdateTourRequest req,
        CancellationToken ct)
    {
        var code = req.Code?.Trim();
        var slug = req.Slug?.Trim();
        ...
        {
            var duplicatedCode = await _db.Tours.IgnoreQueryFilters()
                .AnyAsync(x => x.TenantId == tenantId && x.Code == code && x.Id != currentId, ct);
        ...
        {
            var duplicatedSlug = await _db.Tours.IgnoreQueryFilters()
                .AnyAsync(x => x.TenantId == tenantId && x.Slug == slug && x.Id != currentId, ct);
    ...
    private static QlTourTourDetailDto MapDetail(Tour x)
    {
        return new QlTourTourDetailDto
        {
            Id = x.Id,
            TenantId = x.TenantId,
            ProviderId = x.ProviderId,
...
public sealed class QlTourTourPagedResponse
{
    public int Page { get; set; }
    public int PageSize { get; set; }
    public int Total { get; set; }
    public List<QlTourTourListItemDto> Items { get; set; } = new();
}

public sealed class QlTourTourListItemDto
{
...
public sealed class QlTourTourDetailDto
{
    public Guid Id { get; set; }
...
public sealed class QlTourCreateTourRequest
{
    public Guid? ProviderId { get; set; }
    public Guid? PrimaryLocationId { get; set; }
...
public sealed class QlTourUpdateTourRequest
{
    public Guid? ProviderId { get; set; }
    public Guid? PrimaryLocationId { get; set; }
...
public sealed class QlTourCreateTourResponse
{
    public Guid Id { get; set; }
}
