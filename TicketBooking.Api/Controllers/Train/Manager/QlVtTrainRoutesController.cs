...
// FILE #290: TicketBooking.Api/Controllers/QlVtTrainRoutesController.cs
...
[ApiController]
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/qlvt/train/routes")]
[Authorize(Roles = $"{RoleNames.Admin},{RoleNames.QLVT}")]
public sealed class QlVtTrainRoutesController : ControllerBase
{
    private readonly AppDbContext _db;
    private readonly ITenantContext _tenantContext;

    public QlVtTrainRoutesController(AppDbContext db, ITenantContext tenantContext)
    {
        _db = db;
        ...
        CancellationToken ct = default)
    {
        ...
        if (route is null)
            return NotFound(new { message = "TrainRoute not found in this tenant." });
        ...
        CancellationToken ct = default)
    {
        ...
        if (exists)
            return Conflict(new { message = "TrainRoute Code already exists in this tenant." });
    ...
    public async Task<IActionResult> SoftDelete(Guid id, CancellationToken ct = default)
    {
        ...
        var entity = await _db.TrainRoutes
            .FirstOrDefaultAsync(x => x.Id == id, ct);

        if (entity is null)
            return NotFound(new { message = "TrainRoute not found." });
