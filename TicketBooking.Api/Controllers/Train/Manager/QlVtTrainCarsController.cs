// FILE #292: TicketBooking.Api/Controllers/QlVtTrainCarsController.cs
...
[ApiController]
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/qlvt/train/cars")]
[Authorize(Roles = $"{RoleNames.Admin},{RoleNames.QLVT}")]
public sealed class QlVtTrainCarsController : ControllerBase
{
    private readonly AppDbContext _db;
    private readonly ITenantContext _tenantContext;

    public QlVtTrainCarsController(AppDbContext db, ITenantContext tenantContext)
    {
        _db = db;
        ...
        CancellationToken ct = default)
    {
        ...
        if (car is null)
            return NotFound(new { message = "TrainCar not found in this tenant." });

        var seats = await _db.TrainCarSeats.IgnoreQueryFilters()
            .Where(x => x.CarId == id && x.TenantId == _tenantContext.TenantId && (!x.IsDeleted || includeDeleted))
    ...
    public async Task<IActionResult> Create(
        [FromBody] UpsertTrainCarRequest req,
        CancellationToken ct = default)
    {
        EnsureTenantScope();
        ValidateCar(req);
        ...
        var exists = await _db.TrainCars.IgnoreQueryFilters()
            .AnyAsync(x =>
                ...
                x.TripId == req.TripId &&
                x.CarNumber == req.CarNumber.Trim(), ct);

        if (exists)
            return Conflict(new { message = "CarNumber already exists for this trip." });
    ...
    public sealed class GenerateSeatsRequest
    {
        ...
        public int CompartmentSize { get; set; } = 4;
        public bool SleeperUpperLower { get; set; } = false;
    ...
    public async Task<IActionResult> GenerateSeats(
        Guid carId,
        [FromBody] GenerateSeatsRequest req,
        CancellationToken ct = default)
    {
        EnsureTenantScope();
        ...
        if (req.Rows <= 0 || req.Rows > 50)
            return BadRequest(new { message = "Rows must be 1..50" });

        if (req.Columns <= 0 || req.Columns > 10)
            return BadRequest(new { message = "Columns must be 1..10" });
    ...
    public async Task<IActionResult> SoftDelete(Guid id, CancellationToken ct = default)
    {
        ...
        var entity = await _db.TrainCars
            .FirstOrDefaultAsync(x => x.Id == id, ct);

        if (entity is null)
            return NotFound(new { message = "TrainCar not found." });
