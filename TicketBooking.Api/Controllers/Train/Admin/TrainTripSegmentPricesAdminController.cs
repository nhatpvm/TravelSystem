// FILE #289: TicketBooking.Api/Controllers/TrainTripSegmentPricesAdminController.cs
...
[ApiController]
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/admin/train/trip-segment-prices")]
[Authorize(Roles = RoleNames.Admin)]
public sealed class TrainTripSegmentPricesAdminController : ControllerBase
{
    private readonly AppDbContext _db;
    private readonly ITenantContext _tenantContext;

    public TrainTripSegmentPricesAdminController(AppDbContext db, ITenantContext tenantContext)
    {
        _db = db;
        ...
        CancellationToken ct = default)
    {
        ...
        IQueryable<TrainTripSegmentPrice> query = _db.TrainTripSegmentPrices.Where(x => x.TripId == tripId);
        if (includeDeleted)
    ...
    public async Task<IActionResult> Get(
        ...
        CancellationToken ct = default)
    {
        IQueryable<TrainTripSegmentPrice> query = _db.TrainTripSegmentPrices;
        if (includeDeleted)
            query = query.IgnoreQueryFilters();
        ...
        if (item is null)
            return NotFound(new { message = "TrainTripSegmentPrice not found." });

        var stopRefs = await _db.TrainTripStopTimes.IgnoreQueryFilters()
            .Where(x =>
    ...
    public sealed class UpsertTrainTripSegmentPriceRequest
    {
    ...
    public async Task<IActionResult> Create(
        [FromBody] UpsertTrainTripSegmentPriceRequest req,
        CancellationToken ct = default)
    {
        RequireTenantWrite();
        ValidateUpsert(req);
        ...
        var exists = await _db.TrainTripSegmentPrices.IgnoreQueryFilters().AnyAsync(x =>
        ...
        if (exists)
            return Conflict(new { message = "TrainTripSegmentPrice already exists for this trip segment." });

        var entity = new TrainTripSegmentPrice
        {
            Id = Guid.NewGuid(),
        ...
        _db.TrainTripSegmentPrices.Add(entity);
        ...
        CancellationToken ct = default)
    {
        ...
        var entity = await _db.TrainTripSegmentPrices.IgnoreQueryFilters()
            .FirstOrDefaultAsync(x => x.Id == id && x.TenantId == _tenantContext.TenantId, ct);

        if (entity is null)
            return NotFound(new { message = "TrainTripSegmentPrice not found in this tenant." });
        ...
        if (exists)
            return Conflict(new { message = "Another TrainTripSegmentPrice already exists for this trip segment." });
    ...
    public sealed class ReplaceTrainTripSegmentPricesRequest
    {
        public Guid TripId { get; set; }
        public string CurrencyCode { get; set; } = "VND";
    ...
    public async Task<IActionResult> Replace(
        [FromBody] ReplaceTrainTripSegmentPricesRequest req,
        CancellationToken ct = default)
    {
        RequireTenantWrite();
        ValidateReplace(req);
        ...
        CancellationToken ct = default)
    {
        ...
        if (stopTimes.Count < 2)
            return BadRequest(new { message = "TrainTripStopTimes must contain at least 2 rows before generating segment prices." });
        ...
        var replaceReq = new ReplaceTrainTripSegmentPricesRequest
        {
            TripId = req.TripId,
            CurrencyCode = req.CurrencyCode.Trim().ToUpperInvariant(),
            Items = items.Select(x => new ReplaceTrainTripSegmentPricesRequest.Item
            {
                FromTripStopTimeId = x.FromTripStopTimeId,
                ToTripStopTimeId = x.ToTripStopTimeId,
                BaseFare = x.BaseFare,
    ...
    public async Task<IActionResult> SoftDelete(Guid id, CancellationToken ct = default)
    {
        ...
        var entity = await _db.TrainTripSegmentPrices
            .FirstOrDefaultAsync(x => x.Id == id, ct);
        ...
        _db.TrainTripSegmentPrices.Remove(entity);
        await _db.SaveChangesAsync(ct);
    ...
    public async Task<IActionResult> Restore(Guid id, CancellationToken ct = default)
    {
        ...
        var duplicate = await _db.TrainTripSegmentPrices.IgnoreQueryFilters().AnyAsync(x =>
            x.TenantId == _tenantContext.TenantId &&
    ...
    private static void ValidateUpsert(UpsertTrainTripSegmentPriceRequest req)
    {
    ...
    private static void ValidateReplace(ReplaceTrainTripSegmentPricesRequest req)
    {
        if (req is null) throw new ArgumentNullException(nameof(req));
        if (req.TripId == Guid.Empty) throw new InvalidOperationException("TripId is required.");
        if (string.IsNullOrWhiteSpace(req.CurrencyCode) || req.CurrencyCode.Trim().Length != 3)
    ...
    {
        var refs = await _db.TrainTripStopTimes.IgnoreQueryFilters()
            .Where(x =>
                x.TripId == tripId &&
                x.TenantId == _tenantContext.TenantId &&
                !x.IsDeleted &&
