...
[ApiController]
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/admin/train/car-seats")]
[Authorize(Roles = RoleNames.Admin)]
public sealed class TrainCarSeatsAdminController : ControllerBase
{
    private readonly AppDbContext _db;
    private readonly ITenantContext _tenant;

    public TrainCarSeatsAdminController(AppDbContext db, ITenantContext tenant)
    {
        _db = db;
        _tenant = tenant;
    }
    ...
    [HttpGet("{id:guid}")]
    public async Task<IActionResult> Get([FromRoute] Guid id, [FromQuery] bool includeDeleted = false, CancellationToken ct = default)
    {
        IQueryable<TrainCarSeat> query = _db.TrainCarSeats.AsNoTracking();
        ...
        var seat = await query.FirstOrDefaultAsync(x => x.Id == id, ct);
        if (seat is null) return NotFound(new { message = "TrainCarSeat not found." });
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] UpsertTrainCarSeatRequest req, CancellationToken ct = default)
    {
        EnsureTenantWrite();
        ValidateRequest(req);
        ...
        if (car is null) return BadRequest(new { message = "CarId is invalid for this tenant." });

        if (await HasActiveSeatOccupancyAsync(car.TripId, tenantId, ct))
        ...
        if (existing is not null)
        {
            ...
            return CreatedAtAction(nameof(Get), new { version = "1.0", id = existing.Id }, new { existing.Id });
        }
        ...
        return CreatedAtAction(nameof(Get), new { version = "1.0", id = entity.Id }, new { entity.Id });
    }

    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update([FromRoute] Guid id, [FromBody] UpsertTrainCarSeatRequest req, CancellationToken ct = default)
    {
        EnsureTenantWrite();
        ValidateRequest(req);
        ...
        if (entity is null) return NotFound(new { message = "TrainCarSeat not found." });

        var targetCar = await GetActiveCarAsync(req.CarId, tenantId, ct);
        if (targetCar is null) return BadRequest(new { message = "CarId is invalid for this tenant." });
        ...
        {
            if (await HasActiveSeatOccupancyAsync(tripId, tenantId, ct))
                return Conflict(new { message = TrainSeatOccupancySupport.TripMutationBlockedMessage });
        }
    ...
    public async Task<IActionResult> SoftDelete([FromRoute] Guid id, CancellationToken ct = default)
    {
        ...
        if (car is not null && await HasActiveSeatOccupancyAsync(car.TripId, tenantId, ct))
            return Conflict(new { message = TrainSeatOccupancySupport.TripMutationBlockedMessage });
    ...
    private void EnsureTenantWrite()
    {
    ...
    private Task<bool> HasActiveSeatOccupancyAsync(Guid tripId, Guid tenantId, CancellationToken ct)
