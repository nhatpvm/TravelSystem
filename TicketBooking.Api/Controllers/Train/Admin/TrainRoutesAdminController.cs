// FILE #081: TicketBooking.Api/Controllers/TrainRoutesAdminController.cs
...
[ApiController]
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/admin/train/routes")]
[Authorize(Roles = RoleNames.Admin)]
public sealed class TrainRoutesAdminController : ControllerBase
{
    private readonly AppDbContext _db;
    private readonly ITenantContext _tenantContext;

    public TrainRoutesAdminController(AppDbContext db, ITenantContext tenantContext)
    {
        _db = db;
    ...
    {
        IQueryable<TrainRoute> query = _db.TrainRoutes;
    ...
    public async Task<IActionResult> Get(Guid id, [FromQuery] bool includeDeleted = false, CancellationToken ct = default)
    {
        ...
        var route = await query.FirstOrDefaultAsync(x => x.Id == id, ct);
        if (route is null) return NotFound(new { message = "TrainRoute not found." });

        var stops = await _db.TrainRouteStops.IgnoreQueryFilters()
            .Where(x => x.RouteId == id && !x.IsDeleted)
            .OrderBy(x => x.StopIndex)
            .Select(x => new { x.Id, x.StopIndex, x.StopPointId, x.DistanceFromStartKm, x.MinutesFromStart, x.IsActive })
            .ToListAsync(ct);
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] UpsertRouteRequest req, CancellationToken ct = default)
    {
        RequireTenantWrite();
        Validate(req);

        // provider must exist in this tenant
        ...
        if (!providerOk) return BadRequest(new { message = "ProviderId is invalid for this tenant." });

        await ValidateStopsExistAsync(req.FromStopPointId, req.ToStopPointId, ct);

        var exists = await _db.TrainRoutes.IgnoreQueryFilters().AnyAsync(x =>
            x.TenantId == _tenantContext.TenantId &&
            x.Code == req.Code.Trim(), ct);

        if (exists) return Conflict(new { message = "TrainRoute Code already exists in this tenant." });
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpsertRouteRequest req, CancellationToken ct = default)
    {
        RequireTenantWrite();
        Validate(req);

        var entity = await _db.TrainRoutes.IgnoreQueryFilters()
            .FirstOrDefaultAsync(x => x.Id == id && x.TenantId == _tenantContext.TenantId, ct);

        if (entity is null) return NotFound(new { message = "TrainRoute not found in this tenant." });
    ...
    public sealed class UpsertRouteStopsRequest
    {
    ...
    public async Task<IActionResult> ReplaceStops(Guid id, [FromBody] UpsertRouteStopsRequest req, CancellationToken ct = default)
    {
        RequireTenantWrite();

        var route = await _db.TrainRoutes.IgnoreQueryFilters()
            .FirstOrDefaultAsync(x => x.Id == id && x.TenantId == _tenantContext.TenantId, ct);

        if (route is null) return NotFound(new { message = "TrainRoute not found in this tenant." });
        ...
        var existing = await _db.TrainRouteStops.IgnoreQueryFilters()
            .Where(x => x.RouteId == id && x.TenantId == _tenantContext.TenantId)
            .ToListAsync(ct);

        var existingByIndex = new Dictionary<int, TrainRouteStop>();
        ...
        foreach (var s in ordered)
        {
            ...
            {
                _db.TrainRouteStops.Add(new TrainRouteStop
                {
                    Id = Guid.NewGuid(),
                    TenantId = _tenantContext.TenantId!.Value,
                    RouteId = id,
    ...
    public async Task<IActionResult> SoftDelete(Guid id, CancellationToken ct = default)
    {
        ...
        var entity = await _db.TrainRoutes.FirstOrDefaultAsync(x => x.Id == id, ct);
        if (entity is null) return NotFound(new { message = "TrainRoute not found." });

        _db.TrainRoutes.Remove(entity);
        await _db.SaveChangesAsync(ct);
