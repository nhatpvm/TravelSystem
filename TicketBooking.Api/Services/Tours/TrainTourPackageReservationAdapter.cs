...
public sealed class TrainTourPackageReservationAdapter : ITourPackageSourceReservationAdapter
{
    private readonly AppDbContext _db;

    public TrainTourPackageReservationAdapter(AppDbContext db)
    {
        _db = db;
    }
        ...
        CancellationToken ct = default)
    {
        ...
        if (request.Line.Quantity <= 0)
        {
            return new TourPackageSourceReservationHoldResult
            {
                ...
                ErrorMessage = "Train reservation requires quantity greater than 0."
            };
        }
        ...
        if (!segmentPriceId.HasValue || segmentPriceId.Value == Guid.Empty)
        {
            ...
            {
                Status = TourPackageReservationHoldOutcomeStatus.Failed,
                ErrorMessage = "Train reservation source is missing BoundSourceEntityId."
            };
        }
        ...
        if (segmentPrice is null)
        {
            ...
            {
                Status = TourPackageReservationHoldOutcomeStatus.Failed,
                ErrorMessage = "Train segment price source was not found."
            };
        }
        ...
        if (trip is null || !trip.IsActive || trip.Status != TrainTripStatus.Published)
        {
            return new TourPackageSourceReservationHoldResult
            {
                Status = TourPackageReservationHoldOutcomeStatus.Failed,
                ErrorMessage = "Train trip source is not available for reservation."
            };
        }
        ...
        if (carIds.Count == 0)
        {
            ...
            {
                Status = TourPackageReservationHoldOutcomeStatus.Failed,
                ErrorMessage = "Train trip does not have active cars for reservation."
            };
        }

        var seats = await _db.TrainCarSeats
            .IgnoreQueryFilters()
            .AsNoTracking()
            .Where(x =>
                x.TenantId == trip.TenantId &&
        ...
        var occupiedSeatIds = await _db.TrainTripSeatHolds
            .IgnoreQueryFilters()
            .AsNoTracking()
            .Where(x =>
                x.TenantId == trip.TenantId &&
        ...
        if (selectedSeats.Count < request.Line.Quantity)
        {
            ...
            {
                Status = TourPackageReservationHoldOutcomeStatus.Failed,
                ErrorMessage = "Train trip does not have enough available seats for this package reservation."
            };
        }

        var rows = selectedSeats.Select(seat => new TrainTripSeatHold
        {
            Id = Guid.NewGuid(),
            TenantId = trip.TenantId,
            TripId = trip.Id,
            TrainCarSeatId = seat.Id,
        ...
        return new TourPackageSourceReservationHoldResult
        {
            ...
            SnapshotJson = JsonSerializer.Serialize(new
            {
                ...
                seatIds = rows.Select(x => x.TrainCarSeatId).ToList()
            }),
            Note = $"Held {rows.Count} train seat(s)."
        };
    }
    ...
    {
        var expired = await _db.TrainTripSeatHolds
            .IgnoreQueryFilters()
            .Where(x =>
                x.TenantId == tenantId &&
                x.TripId == tripId &&
        ...
        {
            item.Status = TrainSeatHoldStatus.Expired;
            item.UpdatedAt = now;
        }
