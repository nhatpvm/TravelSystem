...
public static class LockedDemoDataSeed
{
    private const string TenantCode = "NX001";
    private const string CustomerEmail = "customer@ticketbooking.local";

    private const string PaidOrderCode = "DEMO-PAID-001";
    private const string RefundOrderCode = "DEMO-REFUND-001";
    private const string PaidPaymentCode = "PAY-DEMO-PAID-001";
    private const string RefundPaymentCode = "PAY-DEMO-REFUND-001";
    private const string PaidTicketCode = "TKT-DEMO-PAID-001";
    private const string RefundTicketCode = "TKT-DEMO-REFUND-001";
    private const string RefundCode = "REF-DEMO-PARTIAL-001";
    private const string SupportTicketCode = "SUP-DEMO-001";
    private const string SettlementBatchCode = "SET-DEMO-LOCKED-001";
    private const string OnboardingTrackingCode = "ONB-DEMO-PENDING-001";
        ...
        CancellationToken ct = default)
    {
        ...
        if (tenant is null)
        {
            logger.LogWarning("Locked demo seed skipped: tenant {TenantCode} not found.", TenantCode);
            return;
        }
        ...
        if (customer is null)
        {
            logger.LogWarning("Locked demo seed skipped: customer user {Email} not found.", CustomerEmail);
            return;
        }
        ...
        CancellationToken ct)
    {
        ...
        ApplyOrder(
            ...
            subtitle: "Booking đã thanh toán cho demo bảo vệ",
            paidAt,
            settledAt,
            now);
        ...
        ApplyOrder(
            ...
            subtitle: "Booking hoàn tiền một phần cho demo bảo vệ",
            refundPaidAt,
            settledAt,
            now);
        ...
        ApplyTicket(
            ...
            "Ghế B05, B06 - demo partial refund",
        ...
        ApplySettlementLine(
            ...
            "Demo original sale before partial refund",
            settledAt,
            now);
        ...
        ApplySettlementLine(
            ...
            "Demo partial refund adjustment",
            settledAt,
            now);
        ...
        logger.LogInformation(
            "Locked demo seed completed: {PaidOrderCode}, {RefundOrderCode}, {BatchCode}, {SupportTicketCode}.",
            PaidOrderCode,
            RefundOrderCode,
            SettlementBatchCode,
            SupportTicketCode);
        ...
        DateTimeOffset now)
    {
        ...
        order.ContactFullName = string.IsNullOrWhiteSpace(customer.FullName) ? "Customer Demo" : customer.FullName;
        ...
        order.CustomerNote = "Demo locked data for graduation defense.";
        ...
        DateTimeOffset now)
    {
        ...
        payment.ProviderOrderId = $"DEMO-{paymentCode}";
        payment.CurrencyCode = "VND";
        payment.Amount = amount;
        payment.PaidAmount = paidAmount;
        payment.RefundedAmount = refundedAmount;
        payment.RequestPayloadJson = SerializeJson(new { demoLocked = true, provider = "SePay" });
        payment.ProviderResponseJson = SerializeJson(new { demoLocked = true, status = "paid" });
        payment.LastWebhookJson = SerializeJson(new { demoLocked = true, eventName = "payment.paid" });
        ...
        DateTimeOffset now)
    {
        ...
        refund.ReasonText = "Khách đổi kế hoạch, hoàn tiền một phần để demo quy trình đối soát.";
        refund.RefundReference = "DEMO-SEPAY-REFUND-001";
        refund.ReviewNote = "Đã duyệt hoàn tiền một phần cho bộ dữ liệu demo bảo vệ.";
        ...
        DateTimeOffset now)
    {
        ...
        account.AccountHolder = string.IsNullOrWhiteSpace(tenantName) ? "NX001 Demo" : tenantName;
        account.BankBranch = "CN TP.HCM";
        account.Note = "Tài khoản nhận đối soát demo bảo vệ.";
        ...
        DateTimeOffset settledAt)
    {
        ...
        batch.Notes = "Settlement batch demo đã khóa cho buổi bảo vệ.";
        ...
        DateTimeOffset now)
    {
        ...
        line.MetadataJson = SerializeJson(new { demoLocked = true });
        ...
        CancellationToken ct)
    {
        ...
        var legalDocumentFileName = "demo-legal-document.pdf";
        var legalDocumentPath = Path.Combine(onboardingDir, legalDocumentFileName);
        if (!File.Exists(legalDocumentPath))
        {
            await File.WriteAllTextAsync(
                legalDocumentPath,
                "%PDF-1.4\n% Demo legal document placeholder for locked onboarding data.\n",
                ct);
        }
        ...
        var metadata = new
        {
            ...
            serviceType = "hotel",
            businessName = "Demo Riverside Hotel Pending",
            taxCode = "0312345678",
            address = "01 Đường Demo, Quận 1, TP.HCM",
            ...
            legalDocument = new
            {
                originalFileName = "Giay-phep-kinh-doanh-demo.pdf",
        ...
        logger.LogInformation(
            "Locked demo onboarding seed completed: {TrackingCode}.",
