...
const BusTripStopTimesPage = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([getBusManagerOptions(), listBusTrips()])
  ...
  const loadStopTimes = async () => {
    ...
    try {
      const response = await listBusTripStopTimes(selectedTripId, { includeDeleted: true });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (selectedId) {
        await updateBusTripStopTime(selectedId, payload);
        setNotice('Đã cập nhật lịch dừng.');
      } else {
        await createBusTripStopTime(selectedTripId, payload);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      await generateBusTripStopTimesFromRoute(selectedTripId, {
  ...
  const handleToggleDelete = async (item) => {
    ...
    try {
      if (item.isDeleted) {
        await restoreBusTripStopTime(item.id);
        setNotice('Đã khôi phục lịch dừng.');
      } else {
        await deleteBusTripStopTime(item.id);
    ...
    >
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_0.95fr] gap-8">
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
          <div className="px-8 py-6 border-b border-slate-100 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs font-bold text-slate-400 mt-1">Có thể sinh tự động từ các điểm dừng của tuyến rồi chỉnh tay từng chuyến.</p>
            </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 space-y-5">
          ...
          <label className="space-y-2 block">
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Phút từ đầu chuyến</span>
            <input
              type="number"
          ...
          >
            {saving ? 'Đang lưu...' : selectedId ? 'Lưu lịch dừng' : 'Tạo lịch dừng'}
          </button>
        </form>
      </div>
    </BusManagementPageShell>
...
export default BusTripStopTimesPage;
