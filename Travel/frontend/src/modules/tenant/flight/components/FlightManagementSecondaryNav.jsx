...
import { getFlightManagementSectionPath } from '../utils/navigation';

const NAV_ITEMS = [
  { key: 'overview', label: 'Kho vé máy bay', icon: Plane },
  { key: 'operations', label: 'Vận hành', icon: PlaneTakeoff },
  { key: 'providers', label: 'Đội bay', icon: BriefcaseBusiness },
...
const FlightManagementSecondaryNav = ({ currentKey = 'overview' }) => (
  <div className="flex flex-wrap gap-3">
    {NAV_ITEMS.map((item) => {
      ...
      return (
        <NavLink
          key={item.key}
          to={getFlightManagementSectionPath(item.key)}
...
export default FlightManagementSecondaryNav;
