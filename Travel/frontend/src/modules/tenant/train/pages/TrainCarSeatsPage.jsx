...
const TrainCarSeatsPage = () => {
  ...
  const loadCars = async () => {
    try {
      const response = await listTrainCars({ includeDeleted: true });
  ...
  const loadSeats = async () => {
    ...
    try {
      const response = await getTrainCar(selectedCarId, { includeDeleted: true });
      ...
      setError(err.message || 'Không tải được danh sách ghế/giường.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (selectedId) {
        await updateTrainCarSeat(selectedId, payload);
        setNotice('Đã cập nhật ghế/giường.');
      } else {
        await createTrainCarSeat(payload);
  ...
  const handleToggleDelete = async (item) => {
    ...
    try {
      if (item.isDeleted) {
        await restoreTrainCarSeat(item.id);
        setNotice('Đã khôi phục ghế/giường.');
      } else {
        await deleteTrainCarSeat(item.id);
  ...
  return (
    <TrainManagementPageShell
      ...
      subtitle="Chỉnh tay từng chỗ cho toa tàu khi cần tinh chỉnh sau bước sinh layout tự động."
      error={error}
      notice={notice}
      actions={(
        <>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_0.95fr] gap-8">
          ...
          <div className="px-8 py-6 border-b border-slate-100 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs font-bold text-slate-400 mt-1">
                {selectedCar ? `Toa ${selectedCar.carNumber} • ${selectedCar.cabinClass || 'Chưa khai báo hạng'}` : 'Chọn toa tàu để xem chi tiết'}
              </p>
            </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold text-slate-400 mt-2">
                        Khoang: {item.compartmentCode || '--'} • Hàng {item.rowIndex} • Cột {item.columnIndex}
                      </p>
                    </div>
                  </div>
                  <button
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 space-y-5">
          ...
          <label className="space-y-2 block">
            ...
            <select
              value={form.carId || selectedCarId}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label className="space-y-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Mã chỗ</span>
              <input
                value={form.seatNumber}
                onChange={(event) => setForm((current) => ({ ...current, seatNumber: event.target.value }))}
                className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700 outline-none"
                placeholder="01A / K4-01"
                required
              />
            </label>
            <label className="space-y-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Loại chỗ</span>
              <select
                value={form.seatType}
                onChange={(event) => setForm((current) => ({ ...current, seatType: event.target.value }))}
                className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700 outline-none"
              >
                {TRAIN_SEAT_TYPES.map((item) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label className="space-y-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Mã khoang</span>
              <input
                value={form.compartmentCode}
                onChange={(event) => setForm((current) => ({ ...current, compartmentCode: event.target.value }))}
                className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700 outline-none"
                placeholder="K4-01"
              />
            </label>
            <label className="space-y-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Số thứ tự khoang</span>
              <input
                type="number"
                value={form.compartmentIndex}
                onChange={(event) => setForm((current) => ({ ...current, compartmentIndex: event.target.value }))}
                className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700 outline-none"
              />
            </label>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <label className="space-y-2">
              ...
              <input
                type="number"
                value={form.rowIndex}
                onChange={(event) => setForm((current) => ({ ...current, rowIndex: event.target.value }))}
                className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700 outline-none"
              />
            </label>
            <label className="space-y-2">
              ...
              <input
                ...
                onChange={(event) => setForm((current) => ({ ...current, columnIndex: event.target.value }))}
                className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700 outline-none"
              />
            </label>
            <label className="space-y-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Phụ thu</span>
              <input
                type="number"
          ...
          <label className="space-y-2 block">
            ...
            <input
              ...
              onChange={(event) => setForm((current) => ({ ...current, seatClass: event.target.value }))}
              className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700 outline-none"
              placeholder="SoftSeat / K4"
            />
          </label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <label className="inline-flex items-center gap-3 text-sm font-bold text-slate-600">
              <input
                ...
                onChange={(event) => setForm((current) => ({ ...current, isWindow: event.target.checked }))}
            ...
            <label className="inline-flex items-center gap-3 text-sm font-bold text-slate-600">
              <input
                type="checkbox"
                checked={form.isAisle}
                onChange={(event) => setForm((current) => ({ ...current, isAisle: event.target.checked }))}
                className="h-4 w-4 rounded border-slate-300"
              />
              Ghế sát lối đi
            </label>
          ...
          >
            {saving ? 'Đang lưu...' : selectedId ? 'Lưu thay đổi' : 'Tạo chỗ mới'}
          </button>
        </form>
      </div>
    </TrainManagementPageShell>
...
export default TrainCarSeatsPage;
