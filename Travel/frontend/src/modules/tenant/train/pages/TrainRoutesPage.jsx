...
const TrainRoutesPage = () => {
  ...
  const loadRouteDetail = async (routeId, currentItems = items) => {
    ...
    try {
      const detail = await getTrainRoute(routeId, { includeDeleted: true });
  ...
  const loadData = async () => {
    ...
    try {
      const [optionsResponse, listResponse] = await Promise.all([
        getTrainManagerOptions(),
        listTrainRoutes({ includeDeleted: true }),
      ]);
  ...
  const handleSaveRoute = async (event) => {
    ...
    try {
      ...
      if (selectedId) {
        await updateTrainRoute(selectedId, payload);
        setNotice('Đã cập nhật tuyến đường.');
      } else {
        await createTrainRoute(payload);
  ...
  const handleSaveStops = async () => {
    ...
    try {
      await replaceTrainRouteStops(selectedId, {
  ...
  const handleToggleDelete = async (item) => {
    ...
    try {
      if (item.isDeleted) {
        await restoreTrainRoute(item.id);
        setNotice('Đã khôi phục tuyến đường.');
      } else {
        await deleteTrainRoute(item.id);
  ...
  return (
    <TrainManagementPageShell
      pageKey="routes"
      title="Tuyến đường"
      subtitle="Tuyến đường là nền tảng để sinh lịch dừng, giá chặng và các chuyến tàu marketplace."
      error={error}
      notice={notice}
      actions={(
        <>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_1fr] gap-8">
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 space-y-5">
            ...
            <form onSubmit={handleSaveRoute} className="space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <label className="space-y-2">
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Thời lượng dự kiến (phút)</span>
                  <input
                    type="number"
              ...
              <button
                ...
                className={`w-full rounded-2xl px-5 py-4 text-sm font-black uppercase tracking-widest transition-all ${
                  savingRoute ? 'bg-slate-200 text-slate-500' : 'bg-slate-900 text-white hover:bg-[#1EB4D4]'
                }`}
              >
                {savingRoute ? 'Đang lưu...' : selectedId ? 'Lưu thay đổi' : 'Tạo tuyến đường'}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 space-y-5">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-xl font-black text-slate-900">Danh sách ga dừng</p>
              <p className="text-xs font-bold text-slate-400 mt-1">Tối thiểu 2 ga để hệ thống sinh lịch dừng và giá chặng.</p>
            </div>
            <button
              type="button"
              onClick={() => setRouteStops((current) => [...current, createEmptyStopRow(current.length)])}
              className="px-4 py-3 rounded-2xl bg-slate-100 text-xs font-black uppercase tracking-widest text-slate-700"
            >
              Thêm ga
            </button>
          </div>
          ...
          <div className="space-y-4">
            {routeStops.map((item, index) => (
              <div key={`${index}-${item.stopIndex}`} className="rounded-[2rem] border border-slate-200 bg-slate-50 p-5 space-y-4">
                <div className="flex items-center justify-between gap-4">
                  <p className="text-sm font-black text-slate-900">Ga dừng #{index + 1}</p>
                  {routeStops.length > 2 ? (
                    <button
                      type="button"
                      onClick={() => setRouteStops((current) => current.filter((_, rowIndex) => rowIndex !== index).map((row, rowIndex) => ({ ...row, stopIndex: rowIndex })))}
                ...
                <label className="space-y-2 block">
                  ...
                  <select
                    ...
                    onChange={(event) => setRouteStops((current) => current.map((row, rowIndex) => (
                      rowIndex === index ? { ...row, stopPointId: event.target.value } : row
                    )))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <label className="space-y-2">
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Khoảng cách từ đầu tuyến (km)</span>
                    <input
                      type="number"
                      value={item.distanceFromStartKm}
                      onChange={(event) => setRouteStops((current) => current.map((row, rowIndex) => (
                        rowIndex === index ? { ...row, distanceFromStartKm: event.target.value } : row
                      )))}
                      className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-700 outline-none"
                    />
                  </label>
                  ...
                  <label className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(event) => setRouteStops((current) => current.map((row, rowIndex) => (
                        rowIndex === index ? { ...row, minutesFromStart: event.target.value } : row
                      )))}
                      className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-700 outline-none"
                    />
                  </label>
          ...
          <button
            ...
            className={`w-full rounded-2xl px-5 py-4 text-sm font-black uppercase tracking-widest transition-all ${
              savingStops || !selectedId ? 'bg-slate-200 text-slate-500' : 'bg-slate-900 text-white hover:bg-[#1EB4D4]'
            }`}
          >
            {savingStops ? 'Đang lưu ga dừng...' : 'Lưu danh sách ga dừng'}
          </button>
        </div>
      </div>
    </TrainManagementPageShell>
...
export default TrainRoutesPage;
