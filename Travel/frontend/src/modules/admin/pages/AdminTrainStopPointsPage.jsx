...
import {
  ...
  listAdminTrainStopPoints,
  restoreAdminTrainStopPoint,
  updateAdminTrainStopPoint,
...
export default function AdminTrainStopPointsPage() {
  const {
    tenantId,
    tenants,
    selectedTenantId,
  ...
  async function loadData() {
    ...
    try {
      ...
      setError(requestError.message || 'Không tải được ga tàu ở admin.');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (selectedId) {
        await updateAdminTrainStopPoint(selectedId, payload, tenantId);
        setNotice('Đã cập nhật ga tàu.');
      } else {
        await createAdminTrainStopPoint(payload, tenantId);
  ...
  async function handleToggleDelete(item) {
    ...
    try {
      if (item.isDeleted) {
        await restoreAdminTrainStopPoint(item.id, tenantId);
        setNotice('Đã khôi phục ga tàu.');
      } else {
        await deleteAdminTrainStopPoint(item.id, tenantId);
  ...
  const locationLookup = useMemo(() => Object.fromEntries(locations.map((item) => [item.id, item])), [locations]);

  return (
    <AdminTrainPageShell
      pageKey="stop-points"
      title="Ga tàu toàn hệ thống"
      subtitle="Admin theo dõi và chuẩn hóa điểm ga theo từng tenant để public search và điều hành không lệch dữ liệu."
      tenants={tenants}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_0.95fr] gap-8">
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
          <div className="px-8 py-6 border-b border-slate-100">
            ...
            <p className="text-xs font-bold text-slate-400 mt-1">Admin có thể rà nhanh ga đang dùng, ga tạm ngưng và các bản ghi đã ẩn.</p>
          </div>
          <div className="divide-y divide-slate-50">
            {loading ? (
              <div className="px-8 py-10 text-sm font-bold text-slate-500">Đang tải ga tàu...</div>
            ) : items.length === 0 ? (
              <div className="px-8 py-10 text-sm font-bold text-slate-500">Tenant này chưa có ga tàu nào.</div>
            ) : items.map((item) => (
              <div
                key={item.id}
                role="button"
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 space-y-5">
          <div>
            ...
            <p className="text-xs font-bold text-slate-400 mt-1">Giữ đúng ngôn ngữ và spacing như các màn editor hiện có.</p>
          </div>

          <label className="space-y-2 block">
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Địa điểm liên kết</span>
            <select value={form.locationId} onChange={(event) => setForm((current) => ({ ...current, locationId: event.target.value }))} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700 outline-none" required>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label className="space-y-2">
              ...
              <select value={form.type} onChange={(event) => setForm((current) => ({ ...current, type: event.target.value }))} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700 outline-none">
            ...
            <label className="space-y-2">
              ...
              <input type="number" value={form.sortOrder} onChange={(event) => setForm((current) => ({ ...current, sortOrder: event.target.value }))} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700 outline-none" />
            </label>
          </div>

          <label className="space-y-2 block">
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Tên ga tàu</span>
            <input value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700 outline-none" placeholder="Ga Hà Nội" required />
          </label>

          <label className="space-y-2 block">
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Địa chỉ</span>
            <input value={form.addressLine} onChange={(event) => setForm((current) => ({ ...current, addressLine: event.target.value }))} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700 outline-none" />
          </label>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label className="space-y-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Vĩ độ</span>
              <input type="number" step="any" value={form.latitude} onChange={(event) => setForm((current) => ({ ...current, latitude: event.target.value }))} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700 outline-none" />
            </label>

            <label className="space-y-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Kinh độ</span>
              <input type="number" step="any" value={form.longitude} onChange={(event) => setForm((current) => ({ ...current, longitude: event.target.value }))} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700 outline-none" />
            </label>
          </div>

          <label className="space-y-2 block">
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Ghi chú</span>
            <textarea value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} rows={3} className="w-full rounded-[1.5rem] border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium text-slate-700 outline-none" />
          </label>
          ...
          <button type="submit" disabled={saving || !tenantId} className="w-full px-5 py-4 rounded-2xl bg-slate-900 text-white text-sm font-black uppercase tracking-widest disabled:opacity-60">
            {saving ? 'Đang lưu...' : selectedId ? 'Lưu cập nhật' : 'Tạo ga tàu'}
          </button>
        </form>
      </div>
    </AdminTrainPageShell>
