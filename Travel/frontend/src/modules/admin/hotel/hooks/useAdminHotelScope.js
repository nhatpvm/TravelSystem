...
const STORAGE_KEY = 'admin_hotel_tenant_id';
const HOTEL_TENANT_TYPE = 'Hotel';
...
export default function useAdminHotelScope() {
  ...
  useEffect(() => {
    ...
    async function loadTenants() {
      ...
      try {
        const response = await listAdminTenants({ page: 1, pageSize: 100, type: 5 });
        if (!active) return;

        const items = (response.items || []).filter((item) => item?.type === HOTEL_TENANT_TYPE);
        setTenants(items);

        if (!items.length) {
          setSelectedTenantId('');
          setScopeError('Chưa có tenant khách sạn nào để quản lý.');
          return;
        }
        ...
        setScopeError(error.message || 'Không thể tải danh sách tenant cho module khách sạn.');
        setTenants([]);
        setSelectedTenantId('');
      } finally {
        if (active) {
